\part{Laplace変換}
	\section{複素指数関数入力に対する伝達関数の作用}
		\begin{shadebox}
			...
			連続時間信号$f: \realSpace \to \complexSpace$を次のように定める。
			\[
				f(t) =
				\begin{cases}
				...
				\end{cases}
			\]
			$H: s\in\complexSpace \mapsto H(s) \in \complexSpace$をproperで既約な有理関数とする。
		...
		\end{proof}
		\subsection{系: 正弦波入力に対する伝達関数の作用}
			\begin{shadebox}
				$A>0,\;\omega \in \realSpace$とする。
				連続時間信号$f_1,f_2: \realSpace \to \realSpace$を次のように定める。
				\[
					f_1(t) =
					\begin{cases}
