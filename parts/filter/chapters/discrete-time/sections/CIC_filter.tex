\section{CIC up-sampler}
    ...
    \subsection{CIC up-sampler 補償フィルタ}
        ...
        \cref{equation:CIC up-sampler の周波数特性} が示すように CIC up-sampler の位相特性が線形なので Remez のアルゴリズムでは振幅の補正だけを気にして係数設計してよい（生成される畳み込み型 FIR フィルタ（以下単に「FIR フィルタ」と呼ぶ）の位相特性も線形なので合成系の位相特性も線形となる）。
        \par
