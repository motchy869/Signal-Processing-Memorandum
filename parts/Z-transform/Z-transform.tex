\part{Z変換}
	\chapter{基礎理論}
		\section{最終値定理}
			\begin{shadebox}
				$X(z)\;(z\in\complexSpace)$を離散時間信号$x(n)\;(n \in \integers,\;\forall n<0,x(n)=0)$のZ変換とする。
		...
		\section{複素指数関数入力に対する伝達関数の作用}
			\begin{shadebox}
				$A>0,\;\omega \in \realSpace$とする。
				離散時間信号$x: \realSpace \to \complexSpace$を次のように定める。
				\[
					x(n) =
					\begin{cases}
					...
					\end{cases}
				\]
				$H: z\in\complexSpace \mapsto H(z) \in \complexSpace$を、$1/z$を変数とした有理式として既約であるような有理関数とする。
