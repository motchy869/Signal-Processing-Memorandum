\part{Fourier解析}
	...
	\chapter{Fourier変換}
		...
		\section{逆Fourier変換}
			...
			\[ \mathcal{F}^{-1}(F)(\bm{x}) \coloneqq \integrate{\realNumbers^d}{}{W(\bm{\omega},\bm{x})F(\bm{\omega})}{d}{\bm{\omega}} = (2\pi)^{-d/2} \integrate{\realNumbers^d}{}{\exp (i\bm{\omega}^\top\bm{x})F(\bm{\omega})}{d}{\bm{\omega}} \]
		...
		\section{周波数表示されたFourier変換との関係}
			...
			\subsection{逆変換により元の関数に戻ること}
				...
				\begin{gather*}
					G(\bm{f}) \coloneqq \tilde{\mathcal{F}}(g)(\bm{f}) = (2\pi)^{d/2}\mathcal{F}(g)(\bm{\omega}) \\
					\begin{aligned}
						\tilde{\mathcal{F}}^{-1}(G)(\bm{x}) &= \integrate{\realNumbers^d}{}{G(\bm{f})\exp (i2\pi\bm{f}^\top\bm{x})}{d}{\bm{f}} = \integrate{\realNumbers^d}{}{(2\pi)^{d/2}\mathcal{F}(g)(\bm{\omega})\exp (i\bm{\omega}^\top\bm{x})\det\left(\frac{1}{2\pi}I_d\right)}{d}{\bm{\omega}} \\
						&= (2\pi)^{-d/2}\integrate{\realNumbers^d}{}{\mathcal{F}(g)(\bm{\omega})\exp (i\bm{\omega}^\top\bm{x})}{d}{\bm{\omega}} = \mathcal{F}^{-1}(\mathcal{F}(g))(\bm{x}) = g(\bm{x})
					\end{aligned}
				\end{gather*}
