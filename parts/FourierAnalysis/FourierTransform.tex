		...
		\chapter{離散時間Fourier変換(DTFT)}
			...
			\section{積と畳み込みとの関係}
				...
				\subsection{積のDTFT}
					...
					$f,g$の積のDTFTは次式で求まる。
					\begin{align*}
						\text{DTFT}(fg,\bm{\omega}) &= \sum_{n\in\integers} f(\bm{n})g(\bm{n})\exp(-i(\bm{\omega}\odot\bm{T}_\text{s})^\top\bm{n}) = \sum_{n\in\integers} \text{IDTFT}(F,\bm{n}) g(\bm{n})\exp(-i(\bm{\omega}\odot\bm{T}_\text{s})^\top\bm{n}) \\
						&= \sum_{n\in\integers} \left(\frac{\prod_{k=1}^d T_{\text{s},k}}{(2\pi)^d}\integ{\Omega}{}{F(\tilde{\bm{\omega}})e^{i(\tilde{\bm{\omega}}\odot\bm{T}_\text{s})^\top\bm{n}}}{\tilde{\bm{\omega}}}\right) g(\bm{n})\exp(-i(\bm{\omega}\odot\bm{T}_\text{s})^\top\bm{n}) \\
						&= \frac{\prod_{k=1}^d T_{\text{s},k}}{(2\pi)^d}\integ{\Omega}{}{F(\tilde{\bm{\omega}})\left(\sum_{n\in\integers} g(\bm{n})e^{-i\left((\bm{\omega}-\tilde{\bm{\omega}})\odot\bm{T}_\text{s}\right)^\top\bm{n}}\right)}{\tilde{\bm{\omega}}} \\
