		...
		\chapter{離散時間Fourier変換(DTFT)}
			...
			\section{システムの伝達関数と正弦波入力の関係}
				...
				\begin{proof}
					...
					これを逆変換して
					\begin{align*}
						g(n) &= \text{IDTFT}(G,n) = \frac{\Ts}{2\pi}\integ{-\pi/\Ts}{\pi/\Ts}{H(\omega)\frac{\pi}{\Ts i}\sum_{m\in\integers}\left(e^{i\phi}\delta(\omega-\omega_0 - 2\pi m/\Ts) - e^{-i\phi}\delta(\omega+\omega_0 - 2\pi m/\Ts)\right)}{\omega} \\
						&= \frac{1}{2i}H((\omega_0+\pi)\%2\pi-\pi)
					\end{align*}
				\end{proof}
				\let\Ts\undefined
