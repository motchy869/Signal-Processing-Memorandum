			...
			\section{システムの伝達関数と正弦波入力の関係}
				...
				\begin{proof}
					...
					\begin{align*}
						...
						&= \frac{\Ts}{2\pi}\integ{-\pi/\Ts}{\pi/\Ts}{\sum_{m=-\infty}^\infty \exp(i\omega\Ts m)f(n-m)}{\omega} \\
						&= \frac{\Ts}{2\pi}\integ{-\pi/\Ts}{\pi/\Ts}{\frac{1}{2i}\sum_{m=-\infty}^\infty \exp(i\omega\Ts m)\Bigl(\exp\bigl(i(\omega_0\Ts(n-m)+\phi)\bigr)-\exp\bigl(-i(\omega_0\Ts(n-m)+\phi)\bigr)\Bigr)}{\omega} \\
						%&= \frac{\Ts}{2\pi}\integ{-\pi/\Ts}{\pi/\Ts}{\frac{1}{2i}\sum_{m=-\infty}^\infty \Bigl(\exp\bigl(i(\omega_0\Ts n+\phi)\bigr)\exp\bigl(i\Ts(\omega-\omega_0)m\bigr) - \exp\bigl(-i(\omega_0\Ts n+\phi)\bigr)\exp\bigl(i\Ts(\omega+\omega_0)m\bigr)\Bigr)}{\omega} \\
						&= \frac{\Ts}{2\pi}\int_{-\pi/\Ts}^{\pi/\Ts} \frac{1}{2i}\sum_{m=-\infty}^\infty \Bigl(\exp\bigl(i(\omega_0\Ts n+\phi)\bigr)\exp\bigl(i(\omega-\omega_0)\Ts m\bigr) \\
						&\phantom{=} - \exp\bigl(-i(\omega_0\Ts n+\phi)\bigr)\exp\bigl(i(\omega+\omega_0)\Ts m\bigr)\Bigr) \mathrm{d}\omega \\
						&= \frac{\Ts}{2\pi}\int_{-\pi/\Ts}^{\pi/\Ts} \frac{1}{2i}\sum_{m=-\infty}^\infty \biggl(\exp\bigl(i(\omega_0\Ts n+\phi)\bigr)\frac{2\pi}{\Ts}\delta(\omega-\omega_0 - 2\pi m/\Ts)\\
