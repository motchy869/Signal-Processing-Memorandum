\chapter{離散時間Fourier変換(DTFT)}
    ...
    \section{GaussianノイズのDTFT}
        \subsection{パワー・スペクトラムの性質}
            \label{GaussianノイズのDTFTのパワー・スペクトラム}
        ...
        \subsection{Fourier変換との関係}
            ...
            \ref{GaussianノイズのDTFTのパワー・スペクトラム}の証明と同様にして$Y(\omega)$の実部と虚部はそれぞれ独立に正規分布$N(0,\sigma)$に従い、$|Y(\omega)|^2/\sigma^2$は$\chi_2^2$分布に従うことがわかる。
