\chapter{補間}
    \section{IDTFT を用いた有限長信号の補間}
        ...
        \subsection{方法}
            ...
            \[ \hat{x}(t)\coloneq\left.\IDTFT{\XDTFT}(n)\right|_{n\to t} = \frac{1}{2\pi}\integrate{-\pi}{\pi}{\XDTFT(\Omega)\exp\parens*{i\Omega t}}{}{\Omega} = \sum_{m=-\infty}^\infty h(m)\sinc\pi(t-m) \]
    ...
    \section{IDFT を用いた周期信号の補間}
        ...
        \subsection{式 (1) を得る発想}
            ...
            しかしこれは失敗する。
            例えば $\xd$ が実数値信号であるとき $\XDTFT$ は Hermite 対称である $\parens*{\XDTFT(-k)=\XDTFT(N-k)=\conj{\XDTFT(k)}}$。
