\chapter{補間}
    \section{IDTFT を用いた有限長信号の補間}
        ...
        \subsection{数値例}
            ...
            図 \cref{figure:元の信号と IDTFT による実数時間遅延離散時間信号} 2 通りの $d$ について $h^\dagger$ を描いたものである。
            \begin{figure}[H]
                \centering
                \begin{minipage}{0.49\hsize}
                    \includegraphics[keepaspectratio, scale=0.69]
        ...
        \subsection{等間隔補間信号の周波数スペクトラム}
            ...
            \end{proof}
            \subsubsection{数値例}
                \ref{IDTFT を用いた有限長信号の補間>数値例} の $h$ を $4$ 倍に補間した信号 $h_\mathrm{itp}$ とその DTFT の絶対値を次の図に示す。
                \begin{figure}[H]
                    \centering
                    \begin{minipage}{0.49\hsize}
                        \centering
                    ...
                    \begin{minipage}{0.49\hsize}
                        ...
                        \caption{$h,h_\mathrm{itp}$ の DTFT。横軸は正規化角周波数}
                    \end{minipage}
                \end{figure}
    \section{IDFT を用いた周期信号の補間}
        ...
        \subsection{数値例}
            ...
            次式で定義される 2 次元平面上の閉曲線 $C:r(t)\;;t:0\to 1$ を $1/N_\mathrm{pt} \coloneq 1/33$ 間隔で $t=0$ から $(N_\mathrm{pt}-1)/N_\mathrm{pt}$ まで標本化した離散座標周期列を $r_\mathrm{d}:\integers\to\realNumbers^2$ とする。
            \[
            ...
            \begin{figure}[H]
                ...
                \end{minipage}
            \end{figure}
            次の図はこれを IDFT を使う方法で 4 倍に補間した離散座標周期列 $r_\mathrm{itp}$ である。
            \begin{figure}[H]
                \centering
                \includegraphics[keepaspectratio, scale=0.69]
