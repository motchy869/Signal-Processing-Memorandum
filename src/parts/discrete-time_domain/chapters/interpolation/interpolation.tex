\chapter{補間}
    ...
    %     \subsection{数値例}
    %         次の Mathematica notebook に記されている。：\href{\currfiledir/calc/interpolate_unequally-spaced_samples/interpolate_unequally-spaced_samples.nb}{\escapeUnderscore{interpolate_unequally-spaced_samples.nb}}
    %         \par
    ...
    %         $A$ の特性は前述の (1) である。
    %         次の図中の \escapeUnderscore{x_tld_arr} が推定された $\tilde{x}_i$ であり，\escapeUnderscore{x_tld} が $\tilde{x}(t)$ である。
