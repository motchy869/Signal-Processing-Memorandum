\chapter{入力に0次ホールド機構を加えた連続時間システムのz変換}
    ...
    \section{主張}
        ...
        入力信号をサンプル周期$\Ts>0$で0次ホールドして与えるときの出力を$\yd:\realNumbers\to\complexNumbers$とする。
    ...
    \section{導出}
        ...
        \[ \ud(t) = \sum_{k=0}^\infty u(k\Ts)\parens*{\uH(t-k\Ts) - \uH(t-(k+1)\Ts)} \]
        これのラプラス変換を$\Ud$とすると次式で表される。
        \[ \Ud(s) = \sum_{k=0}^\infty u(k\Ts)\NapierE^{-k\Ts s}\frac{1-\NapierE^{-\Ts s}}{s} \]
        これに対する出力$\yd$のラプラス変換を$\YLplc$とすると，次式である。
        \begin{align*}
            \YLplc(s) &= \sum_{k=0}^\infty u(k\Ts)\NapierE^{-k\Ts s}\frac{1-\NapierE^{-\Ts s}}{s}H(s) = \sum_{k=0}^\infty u(k\Ts)\NapierE^{-k\Ts s}\parens*{1-\NapierE^{-\Ts s}}\Hd(s) \\
        ...
        \begin{align*}
            ...
            &= \sum_{k=0}^\infty \ud(k\Ts)\bracks*{z^{-k}\sum_{n=0}^\infty \hdd(n-k)z^{-(n-k)} - z^{-k-1}\sum_{n=0}^\infty \hdd(n-k-1)z^{-(n-k-1)}} \\
            &= \sum_{k=0}^\infty \ud(k\Ts)\bracks*{z^{-k}\sum_{n=k}^\infty \hdd(n-k)z^{-(n-k)} - z^{-k-1}\sum_{n=k+1}^\infty \hdd(n-k-1)z^{-(n-k-1)}} \\
            &= \parens*{\sum_{k=0}^\infty \ud(k\Ts)z^{-k}}(1-z^{-1})\Hdd(z) = \Udd(z)(1-z^{-1})\Hdd(z)
        \end{align*}