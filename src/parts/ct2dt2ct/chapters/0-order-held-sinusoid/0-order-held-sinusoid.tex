\chapter{0次ホールドされた正弦波の周波数スペクトラム}
    ...
    \section{導出}
        ...
        \[ X(f) = \frac{1}{T} \integrate{0}{T}{x(t)\exp(-i 2\pi f t)}{}{t} = \frac{1}{i 2\pi(f-f_0)T}\parens*{1-\exp\parens*{-i 2\pi(f-f_0)T}} \]
        ...
        \begin{align*}
            ...
            &= \frac{1}{T}\sum_{k=0}^{N-1}\exp(i 2\pi f_0 k\Ts)\frac{1}{i 2\pi f}\exp(-i 2\pi f k\Ts)\parens*{1-\exp(-i 2\pi f \Ts)} \\
            &= \frac{1-\exp(-i 2\pi f\Ts)}{i 2\pi f}\frac{1}{T}\underbrace{\sum_{k=0}^{N-1}\exp(i 2\pi(f_0-f)k\Ts)}_{\mathrm{(A)}} \\
    ...
    \section{数値例}
        ...
        \begin{itemize}
            \item \href{\currfiledir/spectrum_of_zero-order-held-sine-wave.nb}{\robustEscapeUnderscore{spectrum_of_zero\-order\-held\-sine\-wave.nb}}
            \item \href{\currfiledir/spectrum_of_zero_order_held_sine_wave.m}{\robustEscapeUnderscore{spectrum_of_zero_order_held_sine_wave.m}}
        \end{itemize}