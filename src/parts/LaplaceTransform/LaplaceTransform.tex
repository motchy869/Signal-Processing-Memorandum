\part{Laplace変換}
    \chapter{複素指数関数入力に対する伝達関数の作用}
        ...
        \end{shadebox}
        \begin{proof}
            $\Np$を$H(s)$の分母多項式の相異なる零点の個数とし，それら零点を$p_0,\dots,p_{\Np}$とする。
            零点$p_k$の次数を$N_{\mathrm{p},k}$とし，$H(s)$の部分分数展開を
            \[ H(s) = c_0 + \sum_{k=1}^{\Np} \sum_{l=1}^{N_{\mathrm{p},k}} \frac{c_{k,l}}{(s-p_k)^l} \]
            とする。
            ここに$c_0,c_{k,l}\;(k=1,\dots,\Np,l=1,\dots,N_{\mathrm{p},k})$は適当な複素数である。
            $f,g$のLaplace変換をそれぞれ$F,G$とすると$G(s) = H(s)F(s) = A H(s)/(s-i\omega)$である。
            これの部分分数展開に現れる，$1/(s-p_k)^l\;(k=1,\dots,\Np,l=1,\dots,N_{\mathrm{p},k})$に比例する項は逆Laplace変換すると$t^{l-1}\NapierE^{p_k t}$に比例する関数となり，$t\to\infty$で0に収束する。\hfill \rule{10cm}{0.4pt}(1)
            \par
        ...
        \section{系:正弦波入力に対する伝達関数の作用}
            ...
            \begin{proof}
                ...
                \[ F_1(s) = \frac{As}{s^2+\omega^2} = \frac{A}{2}\parens*{\frac{1}{s+i\omega} + \frac{1}{s-i\omega}} \]
                ...
                よってこれらの項の和は
                \begin{align}
                    &\quad \frac{AG(-i\omega)/2}{s+i\omega} + \frac{AG(i\omega)/2}{s-i\omega} = \frac{A}{2}\parens*{\frac{G(-i\omega)}{s+i\omega} + \frac{G(i\omega)}{s-i\omega}} \nonumber\\
                    &= \frac{A}{2}\times\frac{1}{s^2+\omega^2}\parens*{G(-i\omega)(s-i\omega) + G(i\omega)(s+i\omega)} \nonumber\\
                ...
                \[ \frac{1}{2}(G(i\omega)+G(-i\omega)) = |G(i\omega)|\frac{1}{2}\parens*{\NapierE^{i\Arg\parens*{G(i\omega)}} + \NapierE^{-i\Arg\parens*{G(i\omega)}}} = |G(i\omega)|\cos\Arg\parens*{G(i\omega)} \]
                同様に
                \[ \frac{-1}{2i}(G(i\omega)-G(-i\omega)) = -|G(i\omega)|\sin\Arg\parens*{G(i\omega)} \]
                以上より，
                \begin{align*}
                    (1) &= |G(i\omega)|\parens*{\cos\Arg\parens*{G(i\omega)}\frac{As}{s^2+\omega^2} - \sin\Arg\parens*{G(i\omega)}\frac{A\omega}{s^2+\omega^2} } \\
                    g(t) &\sim \ILPLC{(1)}(t) = |G(i\omega)|\parens*{\cos\Arg\parens*{G(i\omega)}\cos\omega t - \sin\Arg\parens*{G(i\omega)}\sin\omega t } \\
                    &= |G(i\omega)|\cos\parens*{\omega t + \Arg\parens*{G(i\omega)}}
                \end{align*}
            \end{proof}