    ...
    \chapter{Heavisideの階段関数}
        \section{積分表示}
            ...
            \begin{proof}
                ...
                $x>0$のとき，積分路を$C_\mathrm{A}: C_1 + C_2,C_1 := [-R,R],C_2 := R\NapierE^{i\theta},\theta:0\to\pi$として$f$を$C_\mathrm{A}$上で積分する。
                留数定理から次式が成り立つ。
                ...
                \par
                $x<0$のとき，積分路を$C_\mathrm{B}: -C_1 + C_3,C_3 := R\NapierE^{i\theta},\theta:-\pi\to 0$として$f$を$C_\mathrm{B}$上で積分する。
                ...
                $C_2$上の積分の評価と同様にして$\integrate{C_3}{}{f(z)}{}{z} \to 0 \text{ as }R\to\infty$であるので$\lim_{R\to\infty}\integrate{-R}{R}{f(z)}{}{z} = 0$である。
                以上より定理の主張が従う。
            \end{proof}
