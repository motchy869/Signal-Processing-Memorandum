\chapter{Fourier変換}
    ...
    \section{周波数表示されたFourier変換との関係}
        ...
        \subsection{逆変換により元の関数に戻ること}
            通常のFourier変換が$\mathcal{F}^{-1}(\FT{f})(\bm{x}) = f(\bm{x})$を満たすことを既知として，周波数表示されたFourier変換が$\tilde{\mathcal{F}}^{-1}(\tilde{\mathcal{F}}(g))(\bm{x}) = g(\bm{x})$を満たすことは$\bm{\omega} = 2\pi\bm{f}$なる変数変換を用いて確かめられる。
            \begin{gather*}
                G(\bm{f}) \coloneq \tilde{\mathcal{F}}(g)(\bm{f}) = (2\pi)^{d/2}\FTwithArg{g}{\bm{\omega}} \\
                \begin{aligned}
                    \tilde{\mathcal{F}}^{-1}(G)(\bm{x}) &= \integrate{\realNumbers^d}{}{G(\bm{f})\exp (i2\pi\bm{f}^\top\bm{x})}{d}{\bm{f}} = \integrate{\realNumbers^d}{}{(2\pi)^{d/2}\FTwithArg{g}{\bm{\omega}}\exp (i\bm{\omega}^\top\bm{x})\det\parens*{\frac{1}{2\pi}I_d}}{d}{\bm{\omega}} \\
    ...
    \section{積と畳み込みとの関係}
        ...
        \subsection{畳み込みのFourier変換}
            ...
            \end{align*}
    \section{実部または虚部のみのFourier変換}
        簡単のため1次元の場合を扱う。
        \begin{shadebox}
            $x:\realNumbers\to\complexNumbers,x_\mathrm{R} = \Re{x},x_\mathrm{I} = \Im{x},X = \mathcal{F}(x),X_\mathrm{R} = \mathcal{F}(x_\mathrm{R}),X_\mathrm{I} = \mathcal{F}(x_\mathrm{I}),\omega\in\realNumbers$とする。
            次が成り立つ。
            \begin{enumerate}
                \item $X_\mathrm{R}(\omega) = \frac{1}{2}X(\omega) + \frac{1}{2}\conj{X(-\omega)}$
                \item $X_\mathrm{I}(\omega) = \frac{1}{2i}X(\omega) - \frac{1}{2i}\conj{X(-\omega)}$
                \item $X(\omega) = X_\mathrm{R}(\omega) + i X_\mathrm{I}(\omega)$
            \end{enumerate}
        \end{shadebox}
        \begin{proof}
            2は1と同様にして示せる。3は1と2を用いて容易に示せる。
            そこで1のみを示す。
            \begin{align*}
                X_\mathrm{R}(\omega) &= \frac{1}{\sqrt{2\pi}}\integrate{-\infty}{\infty}{\frac{1}{2}\parens*{x(t)+\conj{x(t)}}\exp(-i\omega t)}{}{t} = \frac{1}{2}X(\omega) + \frac{1}{2}\times\frac{1}{\sqrt{2\pi}}\integrate{-\infty}{\infty}{\conj{x(t)\exp\parens*{-i(-\omega) t}}}{}{t} \\
    ...
    \section{Fourier変換の関係式の早見表}
        ...
        \begin{itemize}
            ...
            \item $X,Y: x,y$のFourier変換（各周波数表示，周波数表示のどちらであるかは文脈に依る）
        \end{itemize}
        ...
        \begin{table}[H]
            ...
            \begin{tabular}[H]{l|c|c}
                ...
                時間反転+複素共役のFourier変換& $\mathcal{F}(\conj{t\mapsto x(-t)})(\omega) = \conj{X(\omega)}$ & $\mathcal{F}(\conj{t\mapsto x(-t)})(f) = \conj{X(f)}$ \\
                Parsevalの等式& $\integrate{-\infty}{\infty}{x(t)\conj{y(t)}}{}{t} = \integrate{-\infty}{\infty}{X(\omega)\conj{Y(\omega)}}{}{\omega}$ & $\integrate{-\infty}{\infty}{x(t)\conj{y(t)}}{}{t} = \integrate{-\infty}{\infty}{X(f)\conj{Y(f)}}{}{f}$
            \end{tabular}
        \end{table}