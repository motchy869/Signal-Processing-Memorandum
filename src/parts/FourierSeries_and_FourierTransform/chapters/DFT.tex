\chapter{離散Fourier変換(DFT)}
    \section{基底}
        ...
        \[ W(\bm{k},\bm{n}) \coloneq \parens*{\prod_{l=1}^d N_l}^{-1/2} \exp i\parens*{\sum_{l=1}^d \frac{k_l n_l}{N_l}2\pi}\]
    ...
    \section{Hermiteな離散座標信号のDFTは実数である}
        ...
        \begin{proof}
            ...
            \end{align*}
            ここで$\bm{n}_\mathrm{M} \coloneq [N_1,\dots,N_d]^\top$とすると，
            \[ \conj{W(\bm{k},\bm{n})} = W(\bm{k},-\bm{n}) = W(\bm{k},\bm{n}_\mathrm{M}-\bm{n}) \]
            また，$f$のHermite性の仮定より
            \[ \conj{f(\bm{n})} = f(-\bm{n}) = f(\bm{n}_\mathrm{M}-\bm{n}) \]
            以上より
            \begin{align*}
                2\Im{\DFTwithArg{f}{\bm{k}}} &= \sum_{\bm{n}\in\Omega} W(\bm{k},\bm{n}_\mathrm{M}-\bm{n})f(\bm{n}) - \sum_{\bm{n} \in \Omega}W(\bm{k},\bm{n})f(\bm{n}_\mathrm{M}-\bm{n}) \\
                &= \sum_{\bm{n}\in\Omega} W(\bm{k},\bm{n}_\mathrm{M}-\bm{n})f(\bm{n}) - \sum_{\bm{n}\in\Omega} W(\bm{k},\bm{n}_\mathrm{M}-\bm{n})f(\bm{n}) \\
                &\phantom{=} (\{(\bm{n}, \bm{n}_\mathrm{M}-\bm{n}) | \bm{n}\in\Omega\} = \{(\bm{n}_\mathrm{M}-\bm{n}), \bm{n} | \bm{n}\in\Omega\}\text{を用いた}) \\
                &= 0
            \end{align*}
        \end{proof}

    \subsection{系: Hermiteな離散座標信号のIDFTは実数である}
    ...
    \section{巡回畳み込みのDFTはDFTの積に比例する}
        \begin{shadebox}
            ...
            \[ \DFTwithArg{f\circledast g}{\bm{k}} = \parens*{\prod_{l=1}^d N_l}^{1/2}\DFTwithArg{f}{\bm{k}}\DFTwithArg{g}{\bm{k}} \]
        \end{shadebox}
        \begin{proof}
            $\bm{N} \coloneq [N_1,\dots,N_d]^\top$とする。
            \begin{align*}
                \DFTwithArg{f\circledast g}{\bm{k}} &= \sum_{\bm{n}\in\Omega}\conj{W(\bm{k},\bm{n})} \parens*{f\circledast g}(\bm{n}) = \sum_{\bm{n}\in\Omega}\conj{W(\bm{k},\bm{n})} \sum_{\bm{m}\in\Omega} f(\bm{m})g((\bm{n}-\bm{m})\%\bm{N}) \\
                &= \sum_{\bm{m}\in\Omega} f(\bm{m}) \sum_{\bm{n}\in\Omega} \conj{W(\bm{k},\bm{n})}g((\bm{n}-\bm{m})\%\bm{N}) \\
                &= \sum_{\bm{m}\in\Omega} f(\bm{m}) \sum_{\bm{n}\in\Omega} \parens*{\prod_{l=1}^d N_l}^{1/2} \conj{W(\bm{k},\bm{m})} \conj{W(\bm{k},\bm{n}-\bm{m})} g((\bm{n}-\bm{m})\%\bm{N}) \\
                &= \parens*{\prod_{l=1}^d N_l}^{1/2} \sum_{\bm{m}\in\Omega} \conj{W(\bm{k},\bm{m})}f(\bm{m}) \sum_{\bm{n}\in\Omega} \conj{W(\bm{k},(\bm{n}-\bm{m})\%\bm{N})} g((\bm{n}-\bm{m})\%\bm{N}) \\
                &= \parens*{\prod_{l=1}^d N_l}^{1/2} \sum_{\bm{m}\in\Omega} \conj{W(\bm{k},\bm{m})}f(\bm{m}) \sum_{\bm{n}\in\Omega} \conj{W(\bm{k},\bm{n})} g(\bm{n}) \\
                &= \parens*{\prod_{l=1}^d N_l}^{1/2}\DFTwithArg{f}{\bm{k}}\DFTwithArg{g}{\bm{k}}
            \end{align*}
        \end{proof}
    ...
    \label{巡回相関のDFT}
        \begin{shadebox}
            ...
            このとき，次が成り立つ。
            \[ \DFTwithArg{R_{f,g}}{\bm{k}} = \parens*{\prod_{l=1}^d N_l}^{1/2}\DFTwithArg{f}{\bm{k}}\conj{\DFTwithArg{g}{\bm{k}}} \]
        \end{shadebox}
        \begin{proof}
            $\bm{N} \coloneq [N_1,\dots,N_d]^\top$とする。
            \begin{align*}
            ...
            \begin{align*}
                \conj{W(\bm{k},\bm{n})} &= W(\bm{k},-\bm{n}) = \parens*{\prod_{l=1}^d N_l}^{1/2}W(\bm{k},\bm{m}-\bm{n})W(\bm{k},-\bm{m}) \\
                &= \parens*{\prod_{l=1}^d N_l}^{1/2}W(\bm{k},(\bm{m}-\bm{n})\%\bm{N})\conj{W(\bm{k},\bm{m})}
            \end{align*}
            これを式(1)に適用して次式を得る。
            \begin{align*}
                \DFTwithArg{R_{f,g}}{\bm{k}} &= \parens*{\prod_{l=1}^d N_l}^{1/2} \sum_{\bm{m}\in\Omega} \conj{W(\bm{k},\bm{m})}f(\bm{m}) \sum_{\bm{n}\in\Omega}\conj{\conj{W(\bm{k},(\bm{m}-\bm{n})\%\bm{N})}g((\bm{m}-\bm{n})\%\bm{N})} \\
                &= \parens*{\prod_{l=1}^d N_l}^{1/2}\DFTwithArg{f}{\bm{k}}\conj{\DFTwithArg{g}{\bm{k}}}
            \end{align*}
        \end{proof}
    \section{巡回畳み込みのDFT}
        \begin{shadebox}
    ...
    \section{GaussianノイズのDFT}
        \begin{shadebox}
            $F(n) \in \complexNumbers\;(n=0,1,\dots,N-1)$は互いに独立で，複素正規分布$N(0,\sigma^2)$に従うとする$\parens*{\text{p}(f) = \frac{1}{2\pi\sigma^2}\exp\frac{-\Re{f}^2-\Im{f}^2}{2\sigma^2} = \frac{1}{2\pi\sigma^2}\exp\frac{-|f|^2}{2\sigma^2}}$。
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                &= \prod_{i=0}^{N-1}\frac{1}{2\pi\sigma^2} \exp\frac{-|(P\bm{g})[i]|^2}{2\sigma^2} = \parens*{\prod_{i=0}^{N-1}\frac{1}{2\pi\sigma^2}} \exp \sum_{i=0}^{N-1} \frac{-|(P\bm{g})[i]|^2}{2\sigma^2} \\
                &= \parens*{\prod_{i=0}^{N-1}\frac{1}{2\pi\sigma^2}} \exp \frac{-\norm{P\bm{g}}_2^2}{2\sigma^2} = \parens*{\prod_{i=0}^{N-1}\frac{1}{2\pi\sigma^2}} \exp \frac{-\norm{\bm{g}}_2^2}{2\sigma^2} \\
                &= \prod_{i=0}^{N-1}\frac{1}{2\pi\sigma^2} \exp \frac{-|g_i|^2}{2\sigma^2}
            \end{align*}
        \end{proof}
    \section{DTFTをDFTで表す}
        \newcommand*{\XprdDFT}{\mathring{X}_\mathrm{DFT}}
        有限長の信号は周期拡張によってDFTと一対一対応し，かつDTFTとも一対一対応するので，信号が有限長であればDTFTとDFTは一対一対応する。
        一次元の場合について示すが，多次元の場合にも容易に拡張できる。
        \begin{shadebox}
            $L\in\naturalNumbers$とする。
            台が$I\coloneq\{0,1,\dots,L-1\}$に含まれる離散時間信号$x$を周期$L$で周期的に拡張した信号を$\mathring{x}$とする。
            $\XDTFT,\XprdDFT$をそれぞれ$x$のDTFT, $\mathring{x}$のDFTとする。
            次式が成り立つ。
        ...
        \begin{proof}
            ...
            これを式(1)に適用して主張が得られる。
        \end{proof}
        数値計算に於いて，$\abs{k/L-F}\pi\ll 1$のときに式(2)の分数を安定的に計算するには，Taylor展開$(\sin Lx)/\sin x \approx L(1-(L^2-1)x^2/6)$を用いるとよい。
