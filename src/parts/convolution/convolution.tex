\part{畳み込み}
    \chapter{畳み込みの微分}
        ...
        \section{使いどころ}
            ...
            (a)長い入力信号に(b)（それに比べて短い）時間的に変化しない信号（典型的にはFIRフィルタの係数）を畳み込むとき，後者の時間微分（典型的には差分で近似）を予め計算しておいて(a)と畳み込めば，微分のオンライン計算が不要になる。
    \chapter{巡回畳み込み}
        \label{巡回畳み込み}
        ...
        \[ \parens*{f\circledast g}(\bm{n}) \coloneq \sum_{\bm{m} \in\Omega} f(\bm{m})g((\bm{n}-\bm{m})\%\bm{N}) \]
        ...
        \section{巡回畳み込みの可換則}
            ...
            \begin{proof}
                \begin{align}
                    \parens*{g\circledast f}(\bm{n}) &= \sum_{\bm{m}\in\Omega} g(\bm{m})f((\bm{n}-\bm{m})\%\bm{N}) \nonumber \\
                ...
                \[ \parens*{g\circledast f}(\bm{n}) = \sum_{\bm{l}\in\Omega} g((\bm{n}-\bm{l})\%\bm{N})f(\bm{l}) = \parens*{f\circledast g}(\bm{n}) \]
            \end{proof}
        \chapter{諸定理}
