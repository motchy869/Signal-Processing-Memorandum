\chapter{連続時間フィルタ}
    \section{諸注意}
        \subsection{複素係数フィルタは実、虚経路単独の実係数フィルタと後段の重み付き和とは等価でない}
            ...
            入力信号 $x:\realNumbers\to\complexNumbers$ に対して出力信号 $y:\realNumbers\to\complexNumbers$ は $y = a_\text{R}\tilde{h}_\text{R}*\Re{x} + b_\text{R}\tilde{h}_\text{I}*\Im{x} + i(a_\text{I}\tilde{h}_\text{R}*\Re{x} + b_\text{I}\tilde{h}_\text{I}*\Im{x})$ である。
            \par
