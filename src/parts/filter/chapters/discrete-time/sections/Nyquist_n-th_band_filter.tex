\section{Nyquist N-th Band Filter}
    \subsection{定義}
        ...
        係数$h:\integers\to\complexNumbers$のDTFTを$\HDTFT$とする。
        但し引数は正規化角周波数である。
        次の性質を満たすフィルタをNyquist N-th Band Filterと呼ぶ。
        \begin{enumerate}
    ...
    \subsection{性質}
        \begin{itemize}
            ...
            \item $m_1,m_2$を2以上の自然数とし，$m_3=m_1m_2$とする。$m_1$-th Band Filterの係数$h_1$を標本化定理に従って$m_2$倍に補間した後に$1/m_2$を乗じて得られる係数$h_3$は$m_3$-th Band Filterの係数である。
        \end{itemize}
        以下で各性質を導出する。
        \subsubsection{性質1}
            \begin{align*}
            ...
            \end{align*}
        \subsubsection{性質2}
            定義の条件3で$\Omega=\pi/N$として定義の条件2を適用すると$1 = \HDTFT(\pi/N) + \HDTFT(-\pi/N)$である。
            これと定義の条件2より性質が従う。
        \subsubsection{性質3,4}
            定義の条件3の両辺を逆DTFTする。任意の整数$n$に対して次式が成り立つ。
            \begin{align*}
        ...
        \subsubsection{性質5}
            $h_1$のDTFTを$\HDTFTss{1}$とする。
            $h_1$を標本化定理に従って連続時間信号に拡張したものを$\hc$とし，そのFourier変換を$\Hc$とする（引数は正規化角周波数）。
            \ref{DTFTとエイリアシングとの関係}より，$\Hc$は$[-\pi,\pi]$に於いて$(\Ts/\sqrt{2\pi})\HDTFTss{1}$に等しく，$(-\infty,\pi),(\pi,\infty)$に於いて0である。
            $h_3$は$\hcSs{2}:t\mapsto\hc(t/m_2)/m_2$を標本化したものと等しい。
            $\HcSs{2}\coloneq\FT{\hcSs{2}}$とすると，$\HcSs{2}(\Omega) = \Hc(m_2\Omega)$が成り立つ。
            再び\ref{DTFTとエイリアシングとの関係}より，$\DTFTwithArg{h_3}{\Omega} = \HDTFTss{1}(m_2\Omega)$が成り立ち，\ref{Nyquist N-th Band Filter の定義}の条件を満たす。