    ...
    \subsection{DTFT の誤差 2 乗積分最小化}
        ...
        \subsubsection{誤差の重みが一様ではない場合}
            ...
            \begin{proof}
                ...
                証明していないが $A$ は正定であると期待され\footnote{$w$ が偶関数であれば，区間 $[-\pi,\pi]$ に於ける $w$ の Fourier 級数展開を考えると $A$ が巡回行列となって固有値が容易に計算でき，正定性を示せるように思える。試してはない。}，以下ではそう仮定する。
            ...
            \end{proof}
    \subsection{Remez のアルゴリズムを使う方法}
        実数係数の場合に於いて Remez のアルゴリズムが使えることはよく知られている。
        ここでは，複素係数の場合に於いて係数の実部と虚部に独立に Remez のアルゴリズムを適用して係数を得る方法を記す。
        \begin{shadebox}
            フィルタの周波数特性（係数列の DTFT）の理想の形を $\Hideal:\realNumbers\to\complexNumbers$ とする。
            但し $\Hideal$ の引数はサンプル周波数について正規化された角周波数である（$\pi/2$ が Nyquist 周波数に対応する）。
            $\HidealS,\HidealA$ をそれぞれ $\Hideal$ の Hermite 対称成分，反 Hermite 対称成分とする。
            $\HidealS,-i\HidealA$ に対してそれぞれ Remez のアルゴリズムで最適な実数係数を求めて $\hrOpt,\hiOpt:\integers\to\realNumbers$ とする。
        ...
        \begin{proof}
            ...
            $h,\hr,\hi$ の DTFT を $H,\Hr,\Hi$ とする。
            但し $H,\Hr,\Hi$ の引数はサンプル周波数について正規化された角周波数である。
            $H$ の Hermite 対称成分，反 Hermite 対称成分をそれぞれ $\Hs,\Ha$ とする（分解は一意である (\cite{数学備忘録} \ref{1:decomp_into_Hermite_sym_and_anti_Hermite_sym_fxns})）。
            \par
            次式が成り立つ。
            \begin{gather*}
            ...
            \end{gather*}
            任意の関数の Hermite 対称成分，反 Hermite 対称成分への分解は一意である (\cite{数学備忘録} \ref{1:decomp_into_Hermite_sym_and_anti_Hermite_sym_fxns}) から，$\Hr(\Omega) = \Hs(\Omega),\Hi(\Omega) = -i\Ha(\Omega)$ である。
            よって，$\Hr(\Omega)\approx\HidealS,\Hi(\Omega)\approx -i\HidealA$ となるように Remez のアルゴリズムで $\hr,\hi$ を求めればよい。
        \end{proof}
